...
| `submission`                                                            | What the run cannot derive from the sandbox: `{ quarter, archive_sha256, authorship }`. The round travels on the intake form, not `metadata.yaml`; `archive_sha256` is re-derived from the uploaded bytes, never a declared string, because a submission binds to a vendor on that digest; `authorship` is `vendor` or `operator`. Absent values are explicit `null`. |
| `state`                                                                 | `scored` when every scenario was reached, else `incomplete`. A run that did not execute all fifty is reported without a number: a partial run does not produce a low score, it produces a meaningless one — the seven non-completable scenarios pay full credit for a harness that never started, so a dead run still aggregates to roughly 0.14.                     |
